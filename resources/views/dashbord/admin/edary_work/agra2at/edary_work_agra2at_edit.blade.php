<form action="{{ route('admin.edary_work_update_agra2', $agra2->id) }}" method="post" enctype="multipart/form-data">
    @csrf
        ...
        <div class="col-md-4">
            ...
            <div class="input-group flex-nowrap">
                ...
                <input type="number" class="form-control" name="agra2_num" id="agra2_num"
                    value="{{ old('agra2_num', $agra2->agra2_num) }}" aria-describedby="basic-addon3" readonly>
            </div>
            @error('agra2_num')
                ...
                <div class="overflow-hidden flex-grow-1">
                    <select class="form-select rounded-start-0" name="year" id="year"
                        ...
                        @foreach ($years as $item)
                            <option value="{{ $item }}" {{ old('year', $agra2->year) == $item ? 'selected' : '' }}>
                                {{ $item }}</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="col-md-4">
            ...
            <div class="input-group flex-nowrap">
                ...
                <input type="date" class="form-control" name="agra2_date" id="agra2_date"
                    value="{{ old('agra2_date', $agra2->agra2_date) }}" aria-describedby="basic-addon3">
            </div>
            @error('agra2_date')
                <span class="invalid-feedback d-block" role="alert">{{ $message }}</span>
            @enderror
        ...
        <div class="col-md-4">
            ...
            <div class="input-group flex-nowrap">
                ...
                <input type="text" class="form-control" name="agra2_take_place" id="agra2_take_place"
                    value="{{ old('agra2_take_place', $agra2->agra2_take_place) }}" aria-describedby="basic-addon3">
            </div>
                ...
                <div class="overflow-hidden flex-grow-1">
                    <select class="form-select rounded-start-0" name="lawyer_id" id="lawyer_id"
                        ...
                        @foreach ($emps as $item)
                            <option value="{{ $item->id }}" {{ old('lawyer_id', $agra2->lawyer_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->employee }}
                            </option>
                        @endforeach
                    </select>
    ...
    <div class="col-md-12 row" style="margin-top: 20px">
        <div class="col-md-12">
            ...
            <div class="input-group flex-nowrap">
                <textarea class="form-control" name="alagra2" id="alagra2" rows="4">{{ old('alagra2', $agra2->alagra2) }}</textarea>
            </div>
            @error('alagra2')
                <span class="invalid-feedback d-block" role="alert">{{ $message }}</span>
            @enderror
