...
<form  method="post" action="{{ route('admin.edary_work_add_files',$all_data->id) }}" enctype="multipart/form-data">
    @csrf
    <div class="row col-md-12 ">
        <div class="col-md-4" >
            <label for="basic-url"class="form-label">{{translate('attachment_name')}}</label>
            <div class="input-group flex-nowrap ">
                <span class="input-group-text" id="basic-addon3"><i class="bi bi-file-earmark-text fs-2"></i></span>
    ...
    <div class="modal fade" tabindex="-1" id="myModal">
        <div class="modal-dialog " style="width: 120%;">
            <div class="modal-content">
                ...
                <form id="form"  method="post" action="{{ route('admin.edary_work_add_archive',$all_data->id) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6">
                                <label for="type" class="form-label">{{ translate('archive_type') }}</label>
                                <select class="form-select"  name="type" id="type" data-control="select2" data-show-subtext="true" data-live-search="true" data-placeholder="{{ translate('select') }}">
                                    ...
                                    @foreach($types as $key=>$value)
                                        <option value="{{ $key }}" @if($key == 'edary') selected @endif>{{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="col-md-6">
                                <label for="secret_degree" class="form-label">{{ translate('secret_degree') }}</label>
