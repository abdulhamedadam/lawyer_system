        ...
        <div id="kt_app_content_container" class="t_container">
            <div class="card shadow-sm" style="border-top: 3px solid #007bff;">
                <div class="card-header">
                    <h3 class="card-title"></i> {{ translate('edit_masrofat') }}</h3>
                ...
                <form action="{{ route('admin.update_masrofat',$all_data->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select class="form-select rounded-start-0" name="case_id" id="case_id" data-placeholder="{{ translate('select') }}">
                                            ...
                                            @foreach($cases as $item)
                                                <option value="{{ $item->id }}" {{ old('case_id',$all_data->related_to_case) == $item->id ? 'selected' : '' }}>{{ $item->case_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select class="form-select rounded-start-0" name="to_emp_id" id="to_emp_id" data-placeholder="{{ translate('select') }}">
                                            ...
                                            @foreach($employees as $item)
                                                <option value="{{ $item->id }}" {{ old('to_emp_id',$all_data->emp_id) == $item->id ? 'selected' : '' }}>{{ $item->employee }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-12 row" style="margin-top: 10px">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select class="form-select rounded-start-0" name="band_id" id="band_id" data-placeholder="{{ translate('select') }}">
                                            ...
                                            @foreach($masrofat as $item)
                                                <option value="{{ $item->id }}" {{ old('band_id',$all_data->band_id) == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-4" >
                                ...
                                <div class="input-group flex-nowrap ">
                                    ...
                                    <input type="number"  class="form-control " name="value"  id="value" value="{{$all_data->value}}" aria-describedby="basic-addon3" >
                                </div>
                                @error('value')
                                <span class="invalid-feedback d-block" role="alert">{{ $message }}</span>
                                @enderror
