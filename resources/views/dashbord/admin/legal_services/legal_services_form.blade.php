            ...
            <div id="kt_app_content_container" class="t_container" >
                <div class="card shadow-sm " style="border-top: 3px solid #007bff;">
                    <div class="card-header">
                        <h3 class="card-title"></i> {{translate('add_legal_services')}}</h3>
                    ...
                    <form  action="{{ route('admin.store_legal_services') }}" method="post" enctype="multipart/form-data">
                        @csrf
                    <div class="card-body">
                        <div class="col-md-12 row">
                                ...
                                <div class="input-group flex-nowrap ">
                                    ...
                                    <select class="form-select rounded-start-0" name="client_name" id="client_name"   data-placeholder="{{translate('select')}}">
                                        ...
                                        @foreach($clients_names as $name)
                                            <option value="{{$name->id}}" {{ old('client_name') == $name->id ? 'selected' : '' }}>{{$name->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select class="form-select rounded-start-0" name="type_of_service" id="type_of_service"   data-placeholder="{{translate('select')}}">
                                            ...
                                            @foreach($types_of_services as $type)
                                                <option value="{{$type->id}}"{{ old('type_of_service') == $type->id ? 'selected' : '' }}>{{$type->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select class="form-select rounded-start-0" name="esnad_to" id="esnad_to"   data-placeholder="{{translate('select')}}">
                                            ...
                                            @foreach($all_employees as $employee)
                                                <option value="{{$employee->id}}"  {{ old('esnad_to') == $employee->id ? 'selected' : '' }}>{{$employee->employee}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="input-group flex-nowrap ">
                                    ...
                                    <input type="text"  class="form-control " name="cost_of_service"  id="cost_of_service" value="{{old('cost_of_service')}}"  aria-describedby="basic-addon3">
                                </div>
                        ...
                        {{-- <div class=" col-md-12 row" style="margin-top: 10px">
                            ...
                            <div class="col-md-12">
                                <label for="basic-url" class="form-label">{{ translate('legal_services_file') }}</label>
                                <input type="file" class="form-control " name="legal_services_file" id="legal_services_file" value="" aria-describedby="basic-addon3" readonly >
                                @error('legal_services_file')
                                <span class="invalid-feedback d-block" role="alert">{{ $message }}</span>
                                @enderror
                            </div>
