...
<form action="{{ route('admin.add_mo7dareen',$all_data->id) }}" method="post" enctype="multipart/form-data">
    @csrf
        ...
        <div class="col-md-2">
            ...
            <div class="input-group flex-nowrap">
                ...
                <input type="text" class="form-control" name="mo7dareen_num" id="mo7dareen_num"
                       value="{{old('mo7dareen_num')}}" aria-describedby="basic-addon3">
            </div>
            @error('paid_date')
                ...
                <div class="overflow-hidden flex-grow-1">
                    <select class="form-select rounded-start-0" name="lawyer" id="lawyer"
                        ...
                        @foreach($emps as $item)
                            <option
                                value="{{$item->id}}" {{ old('lawyer') == $item->id ? 'selected' : '' }}>{{$item->employee}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="col-md-4">
            ...
            <div class="input-group flex-nowrap">
                ...
                <input type="date" class="form-control" name="session_date" id="session_date"
                       value="{{old('session_date')}}" aria-describedby="basic-addon3">
            </div>
            @error('session_date')
            <span class="invalid-feedback d-block" role="alert">{{ $message }}</span>
            @enderror
