    ...
    <div id="kt_app_content" class="app-content flex-column-fluid" >
        <div id="kt_app_content_container" class="t_container" >
            <div class="card shadow-sm " style="border-top: 3px solid #007bff;">
                <div class="card-header">
                    ...
                    <div class="card-toolbar">
                        <div class="text-center">
                            <a class="btn btn-primary" href="{{ route('admin.cases_data') }}">
                ...
                <form  action="{{ route('admin.update_case',$all_data->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="col-md-12 row">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select class="form-select rounded-start-0" name="client_id" id="client_id"    data-placeholder="{{translate('select')}}">
                                            ...
                                            @foreach($clients as $item)
                                                <option value="{{$item->id}}" {{ old('client_id',$all_data->client_id_fk) == $item->id ? 'selected' : '' }}>{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-4" >
                                ...
                                <div class="input-group flex-nowrap ">
                                    ...
                                    <input type="text"  class="form-control " name="case_num"  id="case_num" value="{{$all_data->case_num}}"  aria-describedby="basic-addon3" readonly>
                                </div>
                                @error('case_num')
                                <span class="invalid-feedback d-block" role="alert">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="col-md-4" >
                                ...
                                <div class="input-group flex-nowrap ">
                                    <span class="input-group-text" id="basic-addon3"><i class="fas fa-address-card fs-2"></i></span>
                                    <input type="text"  class="form-control " name="case_title"  id="case_title" value="{{old('case_title',$all_data->case_name)}}"  aria-describedby="basic-addon3" >
                                </div>
                                @error('case_title')
                                <span class="invalid-feedback d-block" role="alert">{{ $message }}</span>
                                @enderror
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select class="form-select rounded-start-0" name="case_type" id="case_type"    data-placeholder="{{translate('select')}}">
                                            ...
                                            @foreach($case_type as $item)
                                                <option value="{{$item->id}}" {{ old('case_type',$all_data->case_type_fk) == $item->id ? 'selected' : '' }}>{{$item->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select class="form-select rounded-start-0" name="case_status" id="case_status"    data-placeholder="{{translate('select')}}">
                                            ...
                                            @foreach($case_status as $item)
                                                <option value="{{$item->id}}" {{ old('case_status',$all_data->case_status) == $item->id ? 'selected' : '' }}>{{$item->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class=" col-md-12 row" style="margin-top: 10px">
                            <div class="col-md-4" >
                                ...
                                <div class="input-group flex-nowrap ">
                                    <span class="input-group-text" id="basic-addon3"><i class="fas fa-address-card fs-2"></i></span>
                                    <input type="number" step="0.01" class="form-control " name="fees"  id="fees" value="{{old('fees',$all_data->fees)}}"  aria-describedby="basic-addon3" >
                                </div>
                                @error('case_title')
                                <span class="invalid-feedback d-block" role="alert">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="col-md-4" style="">
                                ...
                                <div class="input-group flex-nowrap ">
                                    <span class="input-group-text" id="basic-addon3"><i class="fas fa-address-card fs-2"></i></span>
                                    <input type="date"  class="form-control " name="start_date"  id="start_date" value="{{old('start_date',$all_data->start_date)}}"  aria-describedby="basic-addon3" >
                                </div>
                                @error('start_date')
                                <span class="invalid-feedback d-block" role="alert">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="col-md-12">
                            <div class="form-group text-end" style="margin-top: 27px;">
                                <button type="submit" name="add" value="add" id="save" class="btn btn-success btn-flat ">
