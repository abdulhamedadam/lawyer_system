        ...
        <div id="kt_app_content_container" class="t_container" >
            <div class="card shadow-sm " style="border-top: 3px solid #007bff;">
                <div class="card-header">
                    <h3 class="card-title"></i> {{translate('edit_session')}}</h3>
                    <div class="card-toolbar">
                        <div class="text-center">
                            <a class="btn btn-primary" href="{{ route('admin.sessions') }}">
                ...
                <form action="{{ route('admin.update_session',$session_data->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        <div class="row">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select class="form-select rounded-start-0" name="case_id" id="case_id"    data-placeholder="{{translate('select')}}" >
                                            ...
                                            @foreach($cases as $item)
                                                <option value="{{ $item->id }}" {{ old('case_id',$session_data->case_id == $item->id )? 'selected' : '' }}>{{$item->case_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
