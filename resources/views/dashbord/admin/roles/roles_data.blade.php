        ...
        <div id="kt_app_content_container" class="t_container"  >
            <div class="card shadow-sm" style="border-top: 3px solid #007bff;">
                <div class="card-header">
                    <h3 class="card-title">{{ translate('users_roles') }}</h3>
                    <div class="card-toolbar">
                        <div class="text-center">
                            <a class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalsettings" >
                                <i class="bi bi-plus fs-1"></i> {{ translate('add_roles') }}
                            </a>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title"><?=translate('add_roles')?></h3>
                ...
                <form method="post" action="{{route('admin.add_role')}}" enctype="multipart/form-data" id="form">
                    @csrf
...
@section('js')
        ...
        var KTDatatablesServerSide = function () {
            ...
            var initDatatable = function () {
                table = $("#table1").DataTable({
                    ...
                    stateSave: true,
                    ajax: {
                        url: "{{ route('admin.get_ajex_roles') }}",
                    },
                    columns: [
    ...
    <script>
        function edit_role(id)
        {
            console.log('sss');
            $.ajax({
                url: "{{ route('admin.edit_role', ['id' => '__id__']) }}".replace('__id__', id),
                type: "get",
                dataType: "json",
